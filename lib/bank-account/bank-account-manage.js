...
const addBankAccount = async ({ email, password, userID, bankName }) => {
  ...
  if (!_userID || !authentication) {
    ...
    // Retrive data from n26 account
    const accountData = await n26
      .getAccounts(authentication.access_token)
      .then(data => {
        const { iban, bankName } = data;
        return { iban, bankName };
      });

    // Store access and refresh token
    return bankAccountData.addBankAccount({
      bankAccount: { ...authentication, ...accountData },
      userID: _userID
    });
  }
};
